...
void DaemonInstance::setDirty(const std::string& target) {
  ...
  /* Mark the node and all its outputs dirty.
   * Only recompute the hashes if the node is a source file. */
  node->markDirty(node->isSource());

  /* If this node is generated by a rule, mark it dirty as well because it needs
   * to re-run. */
